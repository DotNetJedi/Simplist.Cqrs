...
{
    [GeneratedCode("http://blogs.msdn.com/b/davidebb/archive/2010/01/18/use-c-4-0-dynamic-to-drastically-simplify-your-private-reflection-code.aspx", "0")]
    ...
    class PrivateReflectionDynamicObject : DynamicObject
    {
        ...
        private const BindingFlags AllInstances = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
        ...
        private static IDictionary<string, IProperty> GetTypeProperties(Type type)
        {
            ...
            foreach (PropertyInfo prop in type.GetProperties(AllInstances).Where(p => p.DeclaringType == type))
            {
                typeProperties[prop.Name] = new Property() { PropertyInfo = prop };
            }

            // Now, add all the fields
            foreach (FieldInfo field in type.GetFields(AllInstances).Where(p => p.DeclaringType == type))
            {
        ...
        private static object InvokeMemberOnType(Type type, object target, string name, object[] args)
        {
            try
            {
                ...
                return type.InvokeMember(
                    name,
                    BindingFlags.InvokeMethod | AllInstances,
                    null,
                    target,
                    args);
            }
